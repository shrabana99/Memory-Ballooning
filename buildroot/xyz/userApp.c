...
void updateIdleBitMap() {
    ...
    ull idlemapBufferSize = 0, x; 
    p = idlemapBuffer;
	while ((x = read(idle, p, BYTE_SIZE)) > 0) {
		p += BYTE_SIZE;
		idlemapBufferSize += BYTE_SIZE;
	}
    // printf("idlemapBufferSize : %llu \n ", totalReadSize);
...
void policyHandler(){
    ...
    // while (curPageNo < totalPageNo) {
    //     if(pageData[curPageNo].val > 0 && idlebitMap[curPageNo] == 1  ){
    ...
    printf("%d pages found for suggestion\n", pagesTosuggest); 
...
int main(int argc, char *argv[])
{
    ...
    // memset(buff, 0, TOTAL_MEMORY_SIZE);
    // Instead of a simple initialization with 0, we'll use F(i) to
    // generate a predictable number at ith index which we can use to
    // check contents of the buffer later in the testcase
